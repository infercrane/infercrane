...
test('release checks require exact revision and convergence', () => {
  const passing = evaluateRelease({deployment:{name:'qwen',active_revision_id:'rev-2'},lifecycle_status:{serving_state:'serving',convergence_state:'converged'}}, 'rev-2');
  ...
  assert.equal(failing.pass, false); assert.equal(failing.failures.length, 4);
});
