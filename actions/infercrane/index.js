...
function output(name, value) {
  ...
  if (!file) return;
  const marker = `infercrane_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  append(file, `${name}<<${marker}\n${value}\n${marker}`);
}
...
async function main() {
  ...
  if (!["plan", "apply", "release-check"].includes(mode))
    throw new Error("mode must be plan, apply, or release-check");
  const cli = await resolveCLI(
    input("cli-path", "auto"),
    input("version", "v1.0.0"),
    path.join(process.env.RUNNER_TEMP ?? os.tmpdir(), "infercrane-action"),
  );
  if (mode === "plan") {
    if (!spec) throw new Error("spec is required for plan");
    const raw = run(cli, ["plan", spec, "--output", "json"]);
    ...
    output(
      "revision",
      plan.changes?.find((change) => change.field === "revision")?.after ?? "unchanged",
    );
  } else if (mode === "apply") {
    if (input("confirm-apply") !== "true")
      throw new Error(
        "apply requires confirm-apply: true and should run in a protected GitHub environment",
      );
    ...
    output("result", "pass");
    output("operation-id", result.operation?.id ?? "");
    output("endpoint", result.endpoint ?? "");
    summary(
      `## InferCrane apply succeeded\n\nStable endpoint: \`${result.endpoint ?? "unknown"}\`\n\nDurable operation: \`${result.operation?.id ?? "unknown"}\``,
    );
  } else {
    if (!deployment || !expectedRevision)
      throw new Error("deployment and exact revision are required for release-check");
    const raw = run(cli, ["status", deployment, "--output", "json"]);
    const view = JSON.parse(raw);
    const passportRaw = run(cli, ["passport", "list", deployment, "--output", "json"]);
    const passports = JSON.parse(passportRaw).data ?? [];
...
main().catch((error) => {
  ...
  process.stderr.write(`::error title=InferCrane delivery::${message.replaceAll("\n", "%0A")}\n`);
  process.exitCode = 1;
});
