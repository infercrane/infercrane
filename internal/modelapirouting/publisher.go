		...
		for _, candidateSource := range source.Candidates {
			...
			if _, strict := p.Adapters.Lookup(candidateSource.Adapter); strict {
				...
				if candidate.TargetBindingID != "" {
					...
					if candidateSource.Adapter == supplieradapter.RunPodVLLMAdapterName || candidateSource.Adapter == supplieradapter.RunPodSGLangLBAdapterName || candidateSource.Adapter == supplieradapter.ZAIAdapterName {
