...
const replicaColumns = `id,tenant_id,deployment_id,ordinal,external_key,lifecycle_state,provider,COALESCE(provider_request_id,''),COALESCE(provider_resource_id,''),COALESCE(endpoint,''),health,provider_details_json::text,last_observed_at,created_at,updated_at`
...
func (s *Store) EnsureReplicaIntent(ctx context.Context, replica domain.Replica) (domain.Replica, bool, error) {
	...
	if err == nil {
		if existing.TenantID != replica.TenantID || existing.DeploymentID != replica.DeploymentID || existing.Ordinal != replica.Ordinal {
	...
	_, err = s.ExecContext(ctx, `INSERT INTO replicas(id,tenant_id,deployment_id,ordinal,external_key,lifecycle_state,provider,health,provider_details_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?::jsonb,?,?)`, replica.ID, replica.TenantID, replica.DeploymentID, replica.Ordinal, replica.ExternalKey, replica.LifecycleState, replica.Provider, replica.Health, replica.ProviderDetails, stamp, stamp)
	if isUniqueViolation(err) {
		existing, lookupErr := s.ReplicaByExternalKey(ctx, replica.Provider, replica.ExternalKey)
		if lookupErr == nil && existing.TenantID == replica.TenantID && existing.DeploymentID == replica.DeploymentID && existing.Ordinal == replica.Ordinal {
			return existing, false, nil
		}
...
func (s *Store) SetReplicaProviderIdentity(ctx context.Context, id, requestID, resourceID string) error {
	result, err := s.ExecContext(ctx, `UPDATE replicas SET provider_request_id=COALESCE(provider_request_id,?),provider_resource_id=COALESCE(provider_resource_id,?),lifecycle_state=CASE WHEN lifecycle_state='pending' THEN 'provisioning' ELSE lifecycle_state END,updated_at=? WHERE id=? AND lifecycle_state!='deleted' AND (provider_request_id IS NULL OR provider_request_id=?) AND (provider_resource_id IS NULL OR provider_resource_id=?)`, null(requestID), null(resourceID), now(), id, null(requestID), null(resourceID))
	if err != nil {
		return err
	}
...
func scanReplicaValue(row replicaScanner) (domain.Replica, error) {
	...
	err := row.Scan(&replica.ID, &replica.TenantID, &replica.DeploymentID, &replica.Ordinal, &replica.ExternalKey, &replica.LifecycleState, &replica.Provider, &replica.ProviderRequestID, &replica.ProviderResourceID, &replica.Endpoint, &replica.Health, &replica.ProviderDetails, &observed, &created, &updated)
	if err != nil {
		return replica, err
	}
