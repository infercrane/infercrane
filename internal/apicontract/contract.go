...
var Routes = []Route{
	...
	{"POST", "/admin/marketplace/receipts", "recordMarketplaceReceipt", "Marketplace administration", "Persist one content-free marketplace request receipt", "Object", "Object", 201, false},
	{"POST", "/admin/marketplace/billing/requests", "lookupMarketplaceBillingRequests", "Marketplace administration", "Reconcile marketplace request costs by channel and request identifier", "Object", "Object", 200, false},
	...
	{"POST", "/managed-deployment-quotes", "quoteManagedDeployment", "Compute", "Quote a time-boxed prepaid InferCrane Cloud deployment", "ManagedDeploymentQuoteRequest", "ManagedDeploymentQuoteEnvelope", 200, false},
	...
	{"GET", "/endpoints/{name}/optimization-readiness", "getEndpointOptimizationReadiness", "Inference optimization", "Turn content-free monitoring into a conservative next profiling action", "", "Object", 200, false},
	...
	{"POST", "/deployments/{name}/traffic-observations", "importTrafficObservations", "Monitoring", "Import bounded content-free production request shapes for monitoring and replay", "TrafficObservationImport", "Object", 201, false},
	...
	{"GET", "/deployments/{name}/replays", "listReplays", "Replay", "List privacy-preserving workload-shape traces for one deployment", "", "ObjectList", 200, false},
	...
	{"POST", "/capacity/probes", "probeLaunchCapacity", "Capacity intelligence", "Compare a current catalog quote with read-only provider launch evidence", "LaunchProbeRequest", "LaunchProbeEnvelope", 200, false},
	...
	{"GET", "/workload-profiles", "listWorkloadProfiles", "Inference optimization", "List content-free public workload priors for day-zero screening", "", "ObjectList", 200, false},
	{"GET", "/workload-profiles/{name}", "getWorkloadProfile", "Inference optimization", "Inspect one immutable public workload prior and its optimizer binding", "", "Object", 200, false},
	...
	{"POST", "/optimization/kernel-opportunities", "planKernelOpportunities", "Inference optimization", "Search reviewed existing kernel sources before bounded custom-kernel experiments", "KernelOpportunityRequest", "Object", 200, false},
	{"GET", "/optimization/accelerator-lab/capabilities", "getAcceleratorLabCapabilities", "Inference optimization", "Inspect exact accelerator, runtime, topology, profiler, and modality adapter contracts", "", "Object", 200, false},
	{"POST", "/optimization/accelerator-lab/runs", "runAcceleratorLab", "Inference optimization", "Queue a budget-bounded profile, isolated build, and exact-target qualification operation", "Object", "OperationEnvelope", 202, true},
	...
	{"GET", "/deployments/{name}/continual-optimization/policy", "getContinualOptimizationPolicy", "Inference optimization", "Inspect the bounded continual optimization policy", "", "Object", 200, false},
	{"PUT", "/deployments/{name}/continual-optimization/policy", "setContinualOptimizationPolicy", "Inference optimization", "Set workload, evidence, experiment budget, reliability, and promotion boundaries", "Object", "Object", 200, false},
	{"POST", "/deployments/{name}/continual-optimization/evaluate", "evaluateContinualOptimization", "Inference optimization", "Evaluate public screening or customer replay evidence and persist a bounded next action", "Object", "Object", 201, false},
	{"GET", "/deployments/{name}/continual-optimization/decisions", "listContinualOptimizationDecisions", "Inference optimization", "List immutable continual optimization decisions", "", "ObjectList", 200, false},
	...
	{"GET", "/sandboxes/capabilities", "getSandboxCapabilities", "Sandboxes", "Inspect the configured private-tenant sandbox provider and its exact implemented capabilities", "", "Object", 200, false},
	{"GET", "/sandboxes", "listSandboxes", "Sandboxes", "List tenant-owned private computers without provider identities or customer content", "", "ObjectList", 200, false},
	{"POST", "/sandboxes", "createSandbox", "Sandboxes", "Create a durable private computer from an approved immutable environment", "Object", "Object", 202, true},
	{"GET", "/sandboxes/usage", "getSandboxUsage", "Sandboxes", "Read content-free private-computer usage; this is not an invoice", "", "Object", 200, false},
	{"GET", "/sandboxes/{id}", "getSandbox", "Sandboxes", "Inspect one tenant-owned private computer", "", "Object", 200, false},
	{"POST", "/sandboxes/{id}/commands", "runSandboxCommand", "Sandboxes", "Run one bounded command and stream base64 NDJSON output through InferCrane", "Object", "Object", 200, false},
	{"PUT", "/sandboxes/{id}/files", "writeSandboxFile", "Sandboxes", "Stream a bounded file into the durable workspace", "Object", "Object", 200, true},
	{"GET", "/sandboxes/{id}/files", "readSandboxFile", "Sandboxes", "Stream a file from the durable workspace", "", "Object", 200, false},
	{"POST", "/sandboxes/{id}/ports/{port}/leases", "createSandboxPortLease", "Sandboxes", "Create a short-lived InferCrane-brokered HTTP preview", "Object", "Object", 201, false},
	{"GET", "/sandboxes/{id}/events", "getSandboxEvents", "Sandboxes", "List content-free private-computer activity", "", "ObjectList", 200, false},
	{"GET", "/sandboxes/{id}/receipt", "getSandboxReceipt", "Sandboxes", "Read redacted execution receipt evidence without provider identity", "", "Object", 200, false},
	{"GET", "/sandbox-previews/{sandbox}/{token}/{path...}", "getSandboxPreview", "Sandboxes", "Proxy a tenant-scoped short-lived private-computer preview", "", "Object", 200, false},
	{"HEAD", "/sandbox-previews/{sandbox}/{token}/{path...}", "headSandboxPreview", "Sandboxes", "Inspect a tenant-scoped short-lived private-computer preview", "", "Empty", 200, false},
	{"POST", "/sandboxes/{id}/pause", "pauseSandbox", "Sandboxes", "Put a private computer to sleep while retaining its workspace", "", "Object", 202, true},
	{"POST", "/sandboxes/{id}/resume", "resumeSandbox", "Sandboxes", "Wake a private computer with its durable workspace attached", "", "Object", 202, true},
	{"DELETE", "/sandboxes/{id}", "deleteSandbox", "Sandboxes", "Delete a private computer and durably clean up its workspace", "", "Object", 202, true},
	...
	{"POST", "/billing/checkout-sessions", "createManagedCheckoutSession", "Billing", "Idempotently create a durable fixed-price hosted checkout without changing the prepaid balance", "ManagedCheckoutRequest", "ManagedCheckoutEnvelope", 201, true},
...
func gpuPriceQueryParameters(public bool) []map[string]any {
	...
	return []map[string]any{
		...
		{"name": "deployment_comparable", "in": "query", "required": false, "description": "Filter to complete instance totals from an accepted pricing authority. This still does not claim launch capacity.", "schema": map[string]any{"type": "boolean"}},
...
func schemas() map[string]any {
	...
	result := map[string]any{
		...
		"DeploymentCreate":            map[string]any{"type": "object", "additionalProperties": false, "required": []string{"name", "model", "cloud", "gpu"}, "properties": map[string]any{"name": map[string]any{"type": "string"}, "endpoint_name": map[string]any{"type": "string", "description": "Stable application endpoint alias. Defaults to the deployment name.", "pattern": `^[a-z0-9](?:[a-z0-9._-]{0,62}[a-z0-9])?$`}, "model": map[string]any{"type": "string"}, "runtime": map[string]any{"type": "string", "default": "vllm", "enum": []string{"vllm", "sglang", "custom-oci"}}, "cloud": map[string]any{"type": "string"}, "provider_adapter": map[string]any{"type": "string", "description": "Exact provider profile for this immutable revision; omit only when the cloud/runtime default is unambiguous."}, "compute_mode": map[string]any{"type": "string", "enum": []string{"elastic", "serverless"}}, "billing_mode": map[string]any{"type": "string", "default": "provider_account", "enum": []string{"provider_account", "customer_wallet"}}, "managed_runtime_seconds": map[string]any{"type": "integer", "minimum": 900, "maximum": 86400, "multipleOf": 60}, "gpu": map[string]any{"type": "string"}, "gpu_count": map[string]any{"type": "integer", "minimum": 1, "maximum": 1024, "default": 1, "description": "Accelerators allocated to each runtime replica."}, "region": map[string]any{"type": "string"}, "port": map[string]any{"type": "integer", "minimum": 1, "maximum": 65535}, "min_replicas": map[string]any{"type": "integer", "minimum": 0}, "max_replicas": map[string]any{"type": "integer", "minimum": 0}, "runtime_version": map[string]any{"type": "string"}, "runtime_args": map[string]any{"type": "array", "items": map[string]any{"type": "string"}}, "workload": ref("RuntimeWorkload"), "serving": ref("ServingTopology"), "model_revision": map[string]any{"type": "string"}, "model_secret_reference_id": map[string]any{"type": "string", "description": "Tenant-scoped server-side credential reference for private or gated model resolution."}}},
		...
		"LaunchProbeEnvelope":         map[string]any{"type": "object", "additionalProperties": false, "required": []string{"request", "catalog_quote", "launch_evidence", "selection_boundary"}, "properties": map[string]any{"request": ref("LaunchProbeRequest"), "catalog_quote": ref("CatalogLaunchQuote"), "launch_evidence": ref("LaunchProbeEvidence"), "selection_boundary": map[string]any{"type": "string"}}},
		"ManagedDeploymentQuoteRequest": map[string]any{"type": "object", "additionalProperties": false, "required": []string{"provider", "gpu", "gpu_count", "compute_mode", "runtime_seconds"}, "properties": map[string]any{
			"provider": map[string]any{"type": "string", "minLength": 1, "maxLength": 64}, "region": map[string]any{"type": "string", "maxLength": 128}, "gpu": map[string]any{"type": "string", "minLength": 1, "maxLength": 128}, "gpu_count": map[string]any{"type": "integer", "const": 1}, "compute_mode": map[string]any{"type": "string", "const": "elastic"}, "runtime_seconds": map[string]any{"type": "integer", "minimum": 900, "maximum": 86400, "multipleOf": 60},
		}},
		"ManagedDeploymentQuote": map[string]any{"type": "object", "additionalProperties": false, "required": []string{"state", "provider", "gpu", "gpu_count", "currency", "retail_hourly_microusd", "reserved_microusd", "runtime_limit_seconds", "cleanup_allowance_seconds", "observed_at", "valid_until"}, "properties": map[string]any{
			"state": map[string]any{"type": "string", "const": "current"}, "provider": map[string]any{"type": "string"}, "region": map[string]any{"type": "string"}, "gpu": map[string]any{"type": "string"}, "gpu_count": map[string]any{"type": "integer", "const": 1}, "currency": map[string]any{"type": "string", "const": "USD"}, "retail_hourly_microusd": map[string]any{"type": "integer", "minimum": 1}, "reserved_microusd": map[string]any{"type": "integer", "minimum": 1}, "runtime_limit_seconds": map[string]any{"type": "integer", "minimum": 900, "maximum": 86400}, "cleanup_allowance_seconds": map[string]any{"type": "integer", "minimum": 60}, "observed_at": map[string]any{"type": "string", "format": "date-time"}, "valid_until": map[string]any{"type": "string", "format": "date-time"},
		}},
		"ManagedDeploymentQuoteEnvelope": map[string]any{"type": "object", "additionalProperties": false, "required": []string{"quote", "wallet", "affordable", "shortfall_microusd", "funding_mode", "reservation_is_charge", "disclosure"}, "properties": map[string]any{
			"quote": ref("ManagedDeploymentQuote"), "wallet": ref("ManagedWallet"), "affordable": map[string]any{"type": "boolean"}, "shortfall_microusd": map[string]any{"type": "integer", "minimum": 0}, "funding_mode": map[string]any{"type": "string", "const": "prepaid"}, "reservation_is_charge": map[string]any{"type": "boolean", "const": false}, "disclosure": map[string]any{"type": "string"},
		}},
		...
		"ProviderConnectionCreate":        map[string]any{"type": "object", "additionalProperties": false, "required": []string{"name", "adapter", "target", "secret_reference_id"}, "properties": map[string]any{"name": map[string]any{"type": "string"}, "adapter": map[string]any{"type": "string", "enum": []string{"openrouter", "openai-compatible-external", "modal", "runpod-serverless-api", "fly-io", "baseten", "fireworks"}}, "target": map[string]any{"type": "string"}, "secret_reference_id": map[string]any{"type": "string"}}},
		"ManagedWallet":                   map[string]any{"type": "object", "additionalProperties": false, "required": []string{"tenant_id", "currency", "balance_microusd", "reserved_microusd", "debt_microusd", "available_microusd", "updated_at"}, "properties": map[string]any{"tenant_id": map[string]any{"type": "string"}, "currency": map[string]any{"type": "string", "const": "USD"}, "balance_microusd": map[string]any{"type": "integer"}, "reserved_microusd": map[string]any{"type": "integer", "minimum": 0}, "debt_microusd": map[string]any{"type": "integer", "minimum": 0}, "available_microusd": map[string]any{"type": "integer"}, "updated_at": map[string]any{"type": "string", "format": "date-time"}}},
		...
		"ManagedWalletLedgerEntry":        map[string]any{"type": "object", "additionalProperties": false, "required": []string{"id", "tenant_id", "kind", "currency", "amount_microusd", "description", "created_at"}, "properties": map[string]any{"id": map[string]any{"type": "string"}, "tenant_id": map[string]any{"type": "string"}, "reservation_id": map[string]any{"type": "string"}, "spend_reservation_id": map[string]any{"type": "string"}, "kind": map[string]any{"type": "string", "enum": []string{"credit", "settlement", "refund"}}, "currency": map[string]any{"type": "string", "const": "USD"}, "amount_microusd": map[string]any{"type": "integer"}, "description": map[string]any{"type": "string"}, "created_at": map[string]any{"type": "string", "format": "date-time"}}},
		...
		"EndpointAdoption":                map[string]any{"type": "object", "additionalProperties": false, "required": []string{"name", "logical_model", "source", "ownership_mode", "runtime"}, "properties": map[string]any{"name": map[string]any{"type": "string"}, "logical_model": map[string]any{"type": "string"}, "upstream_model": map[string]any{"type": "string"}, "url": map[string]any{"type": "string", "format": "uri", "description": "Required unless provider_connection supplies the private endpoint."}, "provider_connection": map[string]any{"type": "string", "description": "Tenant-scoped server-side credential and endpoint reference used for authenticated discovery."}, "source": map[string]any{"type": "string", "enum": []string{"vllm", "openai-compatible"}}, "ownership_mode": map[string]any{"type": "string", "enum": []string{"observe-only", "traffic-managed"}}, "runtime": map[string]any{"type": "string"}, "discover": map[string]any{"type": "boolean", "description": "Ask the control plane to verify /v1/models and resolve the selected model before adoption."}, "connector": map[string]any{"type": "string", "enum": []string{"auto", "vllm", "litellm", "openai-compatible"}}}},
		...
		"TrafficObservation": map[string]any{"type": "object", "additionalProperties": false, "required": []string{"source_request_id", "started_at", "latency_ms", "status_code", "operation", "streaming"}, "properties": map[string]any{
			"source_request_id": map[string]any{"type": "string", "minLength": 1, "maxLength": 255}, "started_at": map[string]any{"type": "string", "format": "date-time"}, "latency_ms": map[string]any{"type": "number", "minimum": 0, "maximum": 86400000}, "ttft_ms": map[string]any{"type": "number", "minimum": 0, "maximum": 86400000}, "queue_ms": map[string]any{"type": "number", "minimum": 0, "maximum": 86400000}, "generation_ms": map[string]any{"type": "number", "minimum": 0, "maximum": 86400000}, "input_tokens": map[string]any{"type": "integer", "minimum": 0, "maximum": 10000000}, "output_tokens": map[string]any{"type": "integer", "minimum": 0, "maximum": 10000000}, "status_code": map[string]any{"type": "integer", "minimum": 100, "maximum": 599}, "operation": map[string]any{"type": "string", "minLength": 1, "maxLength": 64}, "provider": map[string]any{"type": "string", "maxLength": 255}, "runtime": map[string]any{"type": "string", "maxLength": 255}, "request_model": map[string]any{"type": "string", "maxLength": 255}, "response_model": map[string]any{"type": "string", "maxLength": 255}, "semantic_convention_schema": map[string]any{"type": "string", "maxLength": 255}, "streaming": map[string]any{"type": "boolean"}, "retry_count": map[string]any{"type": "integer", "minimum": 0, "maximum": 100}, "error_type": map[string]any{"type": "string", "maxLength": 255}, "session_id_hash": map[string]any{"type": "string", "pattern": "^[a-fA-F0-9]{64}$"}, "parent_session_id_hash": map[string]any{"type": "string", "pattern": "^[a-fA-F0-9]{64}$"}, "shared_prefix_hash": map[string]any{"type": "string", "pattern": "^[a-fA-F0-9]{64}$"}, "tool_pause_ms": map[string]any{"type": "number", "minimum": 0, "maximum": 86400000},
		}},
		"TrafficObservationImport":       map[string]any{"type": "object", "additionalProperties": false, "required": []string{"source", "schema_version", "observations"}, "properties": map[string]any{"source": map[string]any{"type": "string", "enum": []string{"opentelemetry", "datadog", "baseten", "fireworks", "custom"}}, "schema_version": map[string]any{"type": "string", "const": "infercrane.traffic-observations/v1"}, "observations": map[string]any{"type": "array", "minItems": 1, "maxItems": 1000, "items": ref("TrafficObservation")}}},
		...
		"CostEvidenceIngest":             map[string]any{"type": "object", "additionalProperties": false, "required": []string{"source", "currency", "evidence_class", "observed_at", "valid_until", "allocations"}, "properties": map[string]any{"source": map[string]any{"type": "string", "minLength": 1, "maxLength": 255}, "currency": map[string]any{"type": "string", "pattern": "^[A-Z]{3}$"}, "evidence_class": map[string]any{"type": "string", "enum": []string{"measured"}, "description": "Tenant-imported cost evidence is measured. Provider-reported authority is assigned only by provider adapters."}, "observed_at": map[string]any{"type": "string", "format": "date-time"}, "valid_until": map[string]any{"type": "string", "format": "date-time"}, "allocations": map[string]any{"type": "array", "minItems": 1, "maxItems": 128, "items": ref("CostEvidenceValue")}}},
		...
		"BenchmarkRequest": map[string]any{"type": "object", "required": []string{"requests", "concurrency"}, "properties": map[string]any{
			"requests": map[string]any{"type": "integer", "minimum": 1, "maximum": 100000}, "concurrency": map[string]any{"type": "integer", "minimum": 1}, "input_tokens": map[string]any{"type": "integer", "minimum": 1, "maximum": 1000000, "default": 128}, "output_tokens": map[string]any{"type": "integer", "minimum": 1, "maximum": 1000000, "default": 32}, "random_seed": map[string]any{"type": "integer", "default": 17}, "runs": map[string]any{"type": "integer", "minimum": 1, "maximum": 10, "default": 1}, "warmup_requests": map[string]any{"type": "integer", "minimum": 0, "maximum": 100000, "default": 0}, "run_cooldown_seconds": map[string]any{"type": "integer", "minimum": 0, "maximum": 3600, "default": 0}, "arrival_pattern": map[string]any{"type": "string", "enum": []string{"constant", "poisson", "gamma"}}, "request_rate": map[string]any{"type": "number", "exclusiveMinimum": 0}, "revision": map[string]any{"type": "string"}, "streaming": map[string]any{"type": "boolean", "default": true},
			"profile": map[string]any{"type": "string", "enum": []string{"balanced", "buffered", "interactive", "long-context", "long-generation", "overload", "public-decode-heavy", "public-interactive", "public-long-prefill", "throughput"}}, "profile_version": map[string]any{"type": "string", "enum": []string{"benchmark-profile-v1"}},
		...
		"OptimizationProposalRequest": map[string]any{"type": "object", "additionalProperties": false, "required": []string{"model_identity", "provider", "gpu"}, "properties": map[string]any{
			"model_identity": map[string]any{"type": "string", "minLength": 1, "maxLength": 256}, "model_revision": map[string]any{"type": "string", "pattern": "^[a-fA-F0-9]{40,64}$"}, "provider": map[string]any{"type": "string", "minLength": 1, "maxLength": 128}, "region": map[string]any{"type": "string", "maxLength": 128}, "gpu": map[string]any{"type": "string", "minLength": 1, "maxLength": 128}, "gpu_count": map[string]any{"type": "integer", "minimum": 1, "maximum": 1024, "default": 1}, "runtimes": map[string]any{"type": "array", "maxItems": 16, "uniqueItems": true, "items": map[string]any{"type": "string", "minLength": 1, "maxLength": 64}}, "objective": map[string]any{"type": "string", "enum": []string{"interactive", "latency", "throughput", "cost-efficiency"}, "default": "interactive"}, "workload_profile": map[string]any{"type": "string", "enum": []string{"balanced", "buffered", "interactive", "long-context", "long-generation", "overload", "public-decode-heavy", "public-interactive", "public-long-prefill", "throughput"}}, "max_ttft_p95_ms": map[string]any{"type": "number", "minimum": 0}, "max_tpot_p95_ms": map[string]any{"type": "number", "minimum": 0}, "max_error_rate": map[string]any{"type": "number", "minimum": 0, "maximum": 1}, "min_goodput": map[string]any{"type": "number", "minimum": 0}, "min_output_tokens_second": map[string]any{"type": "number", "minimum": 0}, "max_hourly_cost": map[string]any{"type": "number", "minimum": 0}, "include_simulated": map[string]any{"type": "boolean", "default": false}, "workload_source": map[string]any{"type": "string", "enum": []string{"reviewed_default", "public_prior", "customer_observed"}}, "workload_fingerprint": map[string]any{"type": "string", "maxLength": 256}, "target_concurrency": map[string]any{"type": "number", "exclusiveMinimum": 0}, "max_candidates": map[string]any{"type": "integer", "minimum": 1, "maximum": 100, "default": 10},
		}},
		...
		"KernelOpportunityRequest": map[string]any{"type": "object", "additionalProperties": false, "required": []string{"schema_version", "model", "runtime", "hardware", "workload", "profile", "policy"}, "properties": map[string]any{
			"schema_version": map[string]any{"type": "string", "const": "infercrane.kernel-opportunity-request/v1"},
			"model":          map[string]any{"type": "object", "additionalProperties": false, "required": []string{"repository", "revision"}, "properties": map[string]any{"repository": map[string]any{"type": "string", "minLength": 1}, "revision": map[string]any{"type": "string", "pattern": "^[a-fA-F0-9]{40,64}$"}}},
			"runtime":        map[string]any{"type": "object", "additionalProperties": false, "required": []string{"name", "version", "image_digest"}, "properties": map[string]any{"name": map[string]any{"type": "string", "minLength": 1}, "version": map[string]any{"type": "string", "minLength": 1}, "image_digest": map[string]any{"type": "string", "pattern": "^sha256:[a-fA-F0-9]{64}$"}}},
			"hardware":       map[string]any{"type": "object", "additionalProperties": false, "required": []string{"vendor", "accelerator"}, "properties": map[string]any{"vendor": map[string]any{"type": "string", "const": "nvidia"}, "accelerator": map[string]any{"type": "string", "minLength": 1}, "compute_capability": map[string]any{"type": "string"}}},
			"workload":       map[string]any{"type": "object", "additionalProperties": false, "required": []string{"digest", "phase", "batch_size", "concurrency", "input_tokens", "output_tokens"}, "properties": map[string]any{"digest": map[string]any{"type": "string", "pattern": "^sha256:[a-fA-F0-9]{64}$"}, "phase": map[string]any{"type": "string", "enum": []string{"prefill", "decode", "mixed"}}, "batch_size": map[string]any{"type": "integer", "minimum": 1}, "concurrency": map[string]any{"type": "integer", "minimum": 1}, "input_tokens": map[string]any{"type": "integer", "minimum": 1}, "output_tokens": map[string]any{"type": "integer", "minimum": 1}}},
			"profile":        map[string]any{"type": "object", "description": "Exact profiler identity and bounded operator hotspots; request-level monitoring alone is insufficient."},
			"policy":         map[string]any{"type": "object", "description": "Amdahl materiality, target speedup, measured-evidence, and candidate-budget gates."},
		}},
	...
	supportedExternalAdapters := []string{"openrouter", "openai-compatible-external", "modal", "runpod-serverless-api", "fly-io", "baseten", "fireworks"}
	for _, schemaName := range []string{"ProviderConnectionCreate", "ExternalPolicyRequest"} {
		result[schemaName].(map[string]any)["properties"].(map[string]any)["adapter"].(map[string]any)["enum"] = supportedExternalAdapters
	}
	return result
}
