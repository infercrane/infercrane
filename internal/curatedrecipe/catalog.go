...
func qwen38H100ProvisionalProfile() ServingProfile {
	return ServingProfile{
		Name:                "sglang-h100-nextn-k4-provisional",
		DisplayName:         "SGLang H100 NEXTN k4 provisional",
		Description:         "Exact configuration from the qualified FastPath Modal candidate, exposed as a portable starting recipe rather than a cross-provider performance claim.",
		Runtime:             "custom-oci",
		RuntimeVersion:      "0.5.18+71de97b264b04dcd514cf904003028aefe9775c8",
		ComputeMode:         "elastic",
		GPUHint:             "H100",
		CompatibleGPUs:      []string{"H100"},
		GPUCount:            1,
		CloudHint:           "runpod",
		ProviderAdapterHint: "runpod-pods",
		MinReplicas:         1,
		MaxReplicas:         1,
		RuntimeArgs:         []string{},
		EvidenceClass:       configurationEvidence,
		QualificationScope:  configurationScope,
		Limitations: []string{
			"The source candidate passed the FastPath qualification workload on one Modal H100 80GB; importing that document does not qualify a separate InferCrane deployment or provider tuple.",
			"RunPod Pods, RunPod Serverless, cold model download, and any non-H100 accelerator require independent reproduction and qualification.",
			"The ongoing optimization campaign may supersede this provisional candidate; this entry is not recipe-v1 and contains no public-grade performance claim.",
		},
		Workload: runtimecontract.Workload{
			...
			Protocol: runtimecontract.OpenAIProtocol, Port: 30000, ReadinessPath: "/health", ModelsPath: "/v1/models", MetricsPath: "/metrics", Cancellation: "http-disconnect", Drain: "connection", ShutdownGraceSeconds: 120,
		},
	}
}
...
func qwen38BlackwellTP4Profile() ServingProfile {
	return ServingProfile{
		Name:                "custom-oci-blackwell-tep4",
		DisplayName:         "Custom OCI Blackwell TEP4",
		Description:         "Immutable upstream Qwen FP8 runtime candidate on four B200 GPUs using tensor and expert parallelism; it is not a measured performance or capacity claim.",
		Runtime:             "custom-oci",
		RuntimeVersion:      "0.1.dev20073+g8e685d198",
		ComputeMode:         "elastic",
		GPUHint:             "B200",
		CompatibleGPUs:      []string{"B200"},
		GPUCount:            4,
		CloudHint:           "runpod",
		ProviderAdapterHint: "runpod-pods",
		MinReplicas:         1,
		...
		QualificationScope:  configurationScope,
		Limitations: []string{
			"The official FP8 guidance validates TP4/TEP4 on Blackwell trays, but this exact four-B200 RunPod tuple still requires real qualification.",
			"The profile deliberately bounds context at 131K; native 262K requests and one-million-token YaRN require separate memory, latency, and quality qualification.",
			"The Qwen Community License 1.0 is not an OSI open-source license and requires operator review for the intended distribution and service model.",
		},
		Workload: runtimecontract.Workload{
			Image:    "vllm/vllm-openai@sha256:fc120ece0a388cc0aa1caad4a9f1cd92113484ab7ec2fd0efadd62585be05bf8",
			Command:  []string{"vllm", "serve", "${MODEL}", "--revision", "${MODEL_REVISION}", "--host", "0.0.0.0", "--port", "${PORT}", "--tensor-parallel-size", "4", "--enable-expert-parallel", "--gpu-memory-utilization", "0.90", "--max-num-seqs", "256", "--max-model-len", "131072", "--enable-prefix-caching", "--no-enable-flashinfer-autotune", "--enable-auto-tool-choice", "--tool-call-parser", "qwen3_xml", "--reasoning-parser", "qwen3", "--served-model-name", "${MODEL}"},
			Protocol: "openai", Port: 8000, ReadinessPath: "/health", ModelsPath: "/v1/models", MetricsPath: "/metrics", Cancellation: "http-disconnect", Drain: "connection", ShutdownGraceSeconds: 120,
		},
	}
}
...
func glm53BlackwellTP4Profile() ServingProfile {
	return ServingProfile{
		...
		DisplayName:         "Custom OCI Blackwell TP4",
		Description:         "Immutable upstream GLM native-FP8 runtime candidate on four B200 GPUs; it is not a measured performance or capacity claim.",
		Runtime:             "custom-oci",
		RuntimeVersion:      "0.1.dev20051+g487ecf187",
		ComputeMode:         "elastic",
		GPUHint:             "B200",
		CompatibleGPUs:      []string{"B200"},
		GPUCount:            4,
		...
		QualificationScope:  configurationScope,
		Limitations: []string{
			"Upstream documents TP4 on a GB200 tray; a four-B200 RunPod worker has the same GPU generation but remains a separate exact tuple that must pass real qualification.",
			"The profile deliberately bounds context at 131K; the declared one-million-token limit requires separate memory, latency, and quality qualification.",
			"This profile is experimental and must not be represented as one-click production-qualified until exact-tuple evidence exists.",
		},
		Workload: runtimecontract.Workload{
			Image:    "vllm/vllm-openai@sha256:2c6da6c6f16ed15c91e412d896dba13701f25fe1861eaec9ddaa4db34d1d21c4",
			Command:  []string{"vllm", "serve", "${MODEL}", "--revision", "${MODEL_REVISION}", "--host", "0.0.0.0", "--port", "${PORT}", "--tensor-parallel-size", "4", "--kv-cache-dtype", "fp8", "--max-model-len", "131072", "--no-enable-flashinfer-autotune", "--speculative-config", "{\"method\":\"mtp\",\"num_speculative_tokens\":5}", "--tool-call-parser", "glm47", "--reasoning-parser", "glm45", "--enable-auto-tool-choice", "--served-model-name", "${MODEL}"},
			Protocol: "openai", Port: 8000, ReadinessPath: "/health", ModelsPath: "/v1/models", MetricsPath: "/metrics", Cancellation: "http-disconnect", Drain: "connection", ShutdownGraceSeconds: 120,
		},
	}
}
...
func init() {
	// Day-zero integration builds can resolve processor or n-gram metadata from
	// a filesystem path before their ordinary Hub resolver has materialized the
	// repository. Use one provider-neutral, immutable snapshot bootstrap for the
	// affected profiles. On persistent storage providers the same command also
	// turns the model cache contract into a reusable exact-model artifact.
	for entryIndex := range catalog {
		...
		if catalog[entryIndex].Name != "glm-5.3-flash" && catalog[entryIndex].Name != "qwen3.8-flash-next" {
			continue
		}
		for profileIndex := range catalog[entryIndex].Profiles {
			...
			profile.Limitations = append(profile.Limitations, "The pinned Hugging Face snapshot is materialized to container or attached persistent storage before vLLM starts; size storage for the exact checkpoint plus runtime overhead.")
		}
	}
}
