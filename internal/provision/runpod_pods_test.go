...
func TestRunPodPodsUsesExactPersistentModelVolumeAndRegion(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/pods":
			...
			if body.NetworkVolumeID != volume.ID || body.VolumeMountPath != "/workspace" || len(body.DataCenterIDs) != 0 || body.Environment["INFERCRANE_MODEL_DIR"] != "/workspace/infercrane/model" || body.Environment["INFERCRANE_MODEL_CACHE_IO"] != "prefetch" || body.Environment["HF_XET_HIGH_PERFORMANCE"] != "1" || body.Environment["HF_TOKEN"] != "{{ RUNPOD_SECRET_infercrane-hf }}" {
