...
func (f *AWSCLI) Run(_ context.Context, _ []string, args ...string) ([]byte, error) {
	...
	case "describe-instances":
		...
		response := map[string]any{"Reservations": []any{map[string]any{"Instances": []any{map[string]any{"InstanceId": f.InstanceID, "PrivateIpAddress": "10.0.0.8", "State": map[string]string{"Name": f.State}, "Tags": []map[string]string{{"Key": "infercrane:external-key", "Value": f.ExternalKey}}}}}}}
