...
func SGLangWorkload() runtimecontract.Workload {
	return runtimecontract.Workload{Image: "lmsysorg/sglang:v0.5.12-runtime@sha256:98223b99a77330dc4bc04cbfc8341e55d874ab911a57d2f508da5a7ef5fb8475", Command: []string{"python3", "-m", "sglang.launch_server", "--model-path", "${MODEL}", "--revision", "${MODEL_REVISION}", "--host", "0.0.0.0", "--port", "${PORT}"}, Protocol: "openai", Port: 8000, ReadinessPath: "/health", ModelsPath: "/v1/models", MetricsPath: "/metrics", Cancellation: "http-disconnect", Drain: "connection", ShutdownGraceSeconds: 30}
}
