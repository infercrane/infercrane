...
class Transport implements ApiTransport {
  ...
  async request(
    method: string,
    path: string,
    options: { body?: JsonValue; idempotencyKey?: string } = {},
  ): Promise<unknown> {
    ...
    try {
      const headers: Record<string, string> = {
        ...
        "User-Agent": "infercrane-typescript/1.0.0",
      };
...
export class InferCrane {
  ...
  constructor(options: InferCraneOptions = {}) {
    ...
    const baseUrl =
      options.baseUrl ?? process.env["INFERCRANE_CONTROL_URL"] ?? "http://127.0.0.1:18000";
    const timeoutMs = options.timeoutMs ?? 30_000;
    const pollIntervalMs = options.pollIntervalMs ?? 1_000;
    if (timeoutMs <= 0 || pollIntervalMs <= 0)
      throw new TypeError("timeoutMs and pollIntervalMs must be positive");
    this.apiKey = apiKey;
  ...
  async deploy(request: DeployRequest): Promise<Operation> {
    const name =
      request.name ?? request.model.split("/").at(-1)!.toLowerCase().replaceAll("_", "-");
    const body: Record<string, JsonValue> = {
      name,
      endpoint_name: request.endpointName ?? name,
      model: request.model,
      cloud: request.cloud,
      gpu: request.gpu,
      runtime: request.runtime ?? "vllm",
      compute_mode: request.computeMode ?? "elastic",
    ...
    const result = (await this.api.createDeployment(
      body,
      request.idempotencyKey ?? `sdk-deploy-${randomUUID()}`,
    )) as { operation: Operation };
    return result.operation;
  }

  async getOperation(id: string): Promise<Operation> {
    return (await this.api.getOperation(id)) as Operation;
  }
  async controlPlaneInstances(): Promise<Record<string, JsonValue>[]> {
    const response = (await this.api.listControlPlaneInstances()) as {
  ...
  async recipes(query = "", limit = 20): Promise<Record<string, JsonValue>[]> {
    ...
    return (
      (await this.transport.request(
        "GET",
        `/recipes?query=${encodeURIComponent(query)}&limit=${limit}`,
      )) as { data: Record<string, JsonValue>[] }
    ).data;
  }

  async lab(
    modelIdentity: string,
    options: { maxTtftP95Ms?: number; workloadDigest?: string } = {},
  ): Promise<Record<string, JsonValue>> {
  ...
  async cancel(id: string): Promise<void> {
    await this.api.cancelOperation(id);
  }
  async getDeployment(name: string): Promise<Deployment> {
    return ((await this.api.getDeployment(name)) as { deployment: Deployment }).deployment;
  }
  async delete(name: string, idempotencyKey = `sdk-delete-${randomUUID()}`): Promise<Operation> {
    return ((await this.api.deleteDeployment(name, idempotencyKey)) as { operation: Operation })
      .operation;
  }
  ...
  async setSloPolicy(
    ...
    policy: {
      ...
      min_output_tokens_second?: number;
    ...
    return (
      (await this.transport.request(
        "PUT",
        `/deployments/${encodeURIComponent(deployment)}/slo-policy`,
        { body: policy as JsonValue },
      )) as { policy: Record<string, JsonValue> }
    ).policy;
  }

  async getSloPolicy(deployment: string): Promise<Record<string, JsonValue>> {
    return (
      (await this.transport.request(
        "GET",
        `/deployments/${encodeURIComponent(deployment)}/slo-policy`,
  ...
  async deleteSloPolicy(deployment: string): Promise<void> {
    await this.transport.request(
      "DELETE",
      `/deployments/${encodeURIComponent(deployment)}/slo-policy`,
    );
  ...
  async recommend(deployment: string): Promise<Record<string, JsonValue>> {
    return (
      (await this.transport.request(
        "POST",
        `/deployments/${encodeURIComponent(deployment)}/recommendations`,
        { body: {} },
      )) as { recommendation: Record<string, JsonValue> }
    ).recommendation;
  }

  async recommendations(deployment: string, limit = 20): Promise<Array<Record<string, JsonValue>>> {
    if (!Number.isInteger(limit) || limit < 1 || limit > 100)
      throw new TypeError("limit must be an integer between 1 and 100");
    return (
      (await this.transport.request(
        "GET",
        `/deployments/${encodeURIComponent(deployment)}/recommendations?limit=${limit}`,
      )) as { data: Array<Record<string, JsonValue>> }
    ).data;
  }
  ...
  async *streamChat(
    ...
    options: { signal?: AbortSignal; parameters?: Record<string, JsonValue> } = {},
  ): AsyncGenerator<Record<string, JsonValue>> {
    const init: RequestInit = {
      ...
      body: JSON.stringify({ model: deployment, messages, stream: true, ...options.parameters }),
    };
    ...
    try {
      while (true) {
        ...
        while ((boundary = buffer.indexOf("\n\n")) >= 0) {
          ...
          try {
            yield JSON.parse(data) as Record<string, JsonValue>;
          } catch {
            throw new StreamError("inference stream contained invalid JSON");
          }
        }
        if (done) break;
      }
