...
class _Transport:
    ...
    def request(self, method: str, path: str, *, body: Any | None = None, idempotency_key: str | None = None) -> Any:
        ...
        headers = {"Accept": "application/json", "Authorization": f"Bearer {self.api_key}", "User-Agent": "infercrane-python/2.0.0rc1"}
...
class InferCrane:
    ...
    def stream_chat(self, deployment: str, messages: list[dict[str, str]], **parameters: Any) -> Iterator[dict[str, Any]]:
        ...
        request = Request(self.gateway_url + "/v1/chat/completions", data=json.dumps(payload, separators=(",", ":")).encode(), headers={"Accept": "text/event-stream", "Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json", "User-Agent": "infercrane-python/2.0.0rc1"}, method="POST")
        try:
