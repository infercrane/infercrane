...
func argumentName(parameter string) string {
	return strings.TrimSuffix(parameter, "...")
}
...
func pythonAPI() string {
	...
	for _, route := range apicontract.Routes {
		...
		for _, parameter := range parameters(route.Path) {
			out.WriteString(", " + argumentName(parameter) + ": str")
		}
		if route.Request != "" {
		...
		if len(parameters(route.Path)) > 0 {
			path := route.Path
			for _, parameter := range parameters(route.Path) {
				path = strings.ReplaceAll(path, "{"+parameter+"}", "{quote("+argumentName(parameter)+", safe='')}")
			}
			out.WriteString("        request_path = f\"" + path + "\"\n")
		} else {
			out.WriteString("        request_path = \"" + route.Path + "\"\n")
		}
		out.WriteString("        return cast(" + responseType + ", self._transport.request(\"" + route.Method + "\", request_path")
		if route.Request != "" {
...
func typeScriptAPI() string {
	...
	for _, route := range apicontract.Routes {
		...
		args := []string{}
		for _, parameter := range parameters(route.Path) {
			args = append(args, argumentName(parameter)+": string")
		}
		if route.Request != "" {
		...
		path := route.Path
		for _, parameter := range parameters(route.Path) {
			path = strings.ReplaceAll(path, "{"+parameter+"}", "${encodeURIComponent("+argumentName(parameter)+")}")
		}
		if len(parameters(route.Path)) > 0 {
			out.WriteString("    const requestPath = `" + path + "`;\n")
		} else {
			out.WriteString("    const requestPath = '" + path + "';\n")
		}
		options := []string{}
		...
		if len(options) > 0 {
			out.WriteString("    return this.transport.request('" + route.Method + "', requestPath, { " + strings.Join(options, ", ") + " }) as Promise<" + responseType + ">;\n")
		} else {
			out.WriteString("    return this.transport.request('" + route.Method + "', requestPath) as Promise<" + responseType + ">;\n")
		}
		out.WriteString("  }\n")
	}
	out.WriteString("}\n")
