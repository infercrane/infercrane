...
`qualified` candidate. An evolution campaign may promote only a `guard_passed` candidate and
delegates the traffic mutation to the normal guarded rollout workflow. Approval expiry is checked
before mutation, while a retry after a completed activation remains a successful no-op.
