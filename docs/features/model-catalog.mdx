...
`glm-5.3-flash` is the first experimental catalog entry exercising this general path. Its reviewed
profile pins the model commit and upstream dedicated vLLM image, requests one four-H200 Kubernetes
replica, and records tensor-parallel argv. It remains **configuration-verified**, not real-GPU or
production-qualified. The profile deliberately uses a bounded 131K context starting point rather
than converting the model card's declared 1M maximum into an unmeasured production promise. See the
[official model card](https://huggingface.co/zai-org/GLM-5.3-Flash) and
[upstream vLLM recipe](https://recipes.vllm.ai/zai-org/GLM-5.3-Flash).
