...
The GLM profile requests one four-H200 Kubernetes replica with tensor parallelism. It deliberately
...
Those commands are local and non-mutating through `plan`. Deployment still requires an operator's
Kubernetes GPU cluster and fresh qualification evidence for the exact generated tuple.
...
Those commands are also local and non-mutating through `plan`. Real deployment requires an exact
eight-H200 worker and fresh protocol, memory, performance, and cleanup evidence.
