...
The GLM profile requests one native RunPod Pod with four H200 GPUs and tensor parallelism. The
`runpod-pods` adapter launches the pinned upstream image directly, without an SSH-derived wrapper.
It deliberately
uses a bounded 131K context starting point rather than converting the model card's declared 1M
...
capacity, enough configured container disk, paid-resource acknowledgement, and fresh qualification
evidence for the exact generated tuple.
