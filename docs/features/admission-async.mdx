...
| `429 admission_rejected` | Endpoint concurrency/queue full or queue timeout before upstream transmission | Preserve `X-Request-ID`, honor `Retry-After`, compare the error message with `admission get`, and inspect the rejection row. |
