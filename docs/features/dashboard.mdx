...
**New inference** starts from the user's existing situation instead of requiring infrastructure
migration:

1. **Use a model API** registers OpenRouter or another OpenAI-compatible provider. Configuration
...
3. **Deploy a model** starts from a reviewed model configuration and then asks for explicit
   infrastructure, runtime, accelerator, compute mode, region, and replica bounds.
