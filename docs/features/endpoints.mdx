...
promoted. Candidate traffic is never duplicated silently; v1.1 evaluates trustworthy telemetry
from the primary deployment-backed binding and reports insufficient evidence as `INCONCLUSIVE`.

## Ownership boundaries

Bindings record `observe-only`, `traffic-managed`, or `lifecycle-managed` ownership. v1.1 supports
fully compiled lifecycle-managed deployment bindings. Incremental import/adopt workflows and
customer-managed external bindings are the v1.2 scope; creating a record does not imply that
