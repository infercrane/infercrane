...
The private browser console exposes the reviewed configuration catalog under **New inference →
Deploy a model**. Selecting a template prefills the model and runtime while leaving infrastructure,
accelerator, compute mode, region, and replica bounds explicit. A curated template remains
configuration-only until that exact serving plan has measured benchmark evidence.
