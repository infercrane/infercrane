...
<div className="ic-hero not-prose">
  <div className="ic-kicker"><span className="ic-status-dot"></span> v0.5 release candidate development</div>
...
declared and conformance-tested; each adapter still has an independent qualification status. The
first operational target remains vLLM on RunPod, while narrow AWS BYOC support is experimental.
Consult [capability status](/project-status) before relying on an adapter.
</Note>
