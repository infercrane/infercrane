...
```bash Send a streaming request
infercrane request support-production \
  --message "Summarize this incident." \
  --stream
```

Build a deployment from a model or custom OCI workload and expose one OpenAI-compatible logical
endpoint. Durable control-plane operations provision, reconcile, scale, and safely replace workers
behind it. If inference already exists, adopt it later without migrating first. Providers and
runtimes integrate through explicit contracts; Kubernetes is not required.
