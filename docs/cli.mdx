...
assumption and identity probe for the configured AWS BYOC adapter. Diagnostics are read-only.
...
| `--cloud` | `runpod` on primary path | elastic/serverless | Qualified provider cloud; `aws` is elastic-only in v0.3 |
| `--gpu` | `L40S` on primary path | elastic/serverless | Requested GPU |
| `--region` | provider-selected | provisioned | Provider region; required for AWS BYOC |
