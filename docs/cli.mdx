...
- Mutation commands accept `--idempotency-key`; reuse the same key after an uncertain result.
- `--wait` polls persisted operation state. Closing the client does not cancel server-side work.
