...
```bash
export INFERCRANE_QUALIFICATION_RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$(git rev-parse --short HEAD)-runpod"
...
`INFERCRANE_QUALIFICATION_RUN_ID`. Do not generate another ID.
...
for suffix in serverless serverless-faults elastic elastic-faults; do
  INFERCRANE_ACCEPTANCE_RUN_ID="$INFERCRANE_QUALIFICATION_RUN_ID-$suffix" \
