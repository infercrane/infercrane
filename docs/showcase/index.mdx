...
<CardGroup cols={3}>
  ...
  <Card title="Compare cost and performance" icon="chart-line" href="/showcase/cost-performance">
    Decide between a model API, self-hosted capacity, and a hybrid plan without inventing prices or savings.
  </Card>
</CardGroup>
...
```mermaid
flowchart LR
  A[Deploy or connect] --> B[Observe requests and operations]
...
evidence states. Check [compatibility and qualification](/compatibility) before relying on a combination in
production.
</Note>
...
The hermetic demo exercises adoption, a real proxied fixture request, Request Inspector, Doctor,
immutable candidate creation, Release Guard rejection, deterministic explanation, and cleanup. It
does not provision a GPU and cannot be used as provider or performance evidence.
