...
<CodeGroup>
```bash gpt-oss
infercrane plan openai/gpt-oss-20b
```

```bash Google Gemma 4
infercrane plan google/gemma-4-26B-A4B-it
```

```bash DeepSeek V4
infercrane plan deepseek-ai/DeepSeek-V4-Pro-0813
```

```bash Moonshot Kimi K3
infercrane plan moonshotai/Kimi-K3
```

```bash Qwen3.8
infercrane plan Qwen/Qwen3.8-2.4T-A95B
```
</CodeGroup>
...
| --- | --- | --- |
| Kimi | Frontier long-context, reasoning, coding, and multimodal work | Custom license, very large MoE topology, and version-specific runtime support |
| DeepSeek | High-adoption reasoning, agents, and cost-sensitive API workloads | Exact Pro/Flash artifact, serving framework, and multi-node plan |
| Qwen | Frontier and compact variants across agents, coding, vision, and chat | Exact chat template, license, tool calling, quantization, and topology |
| GLM | Coding and long-horizon agent workloads | Artifact availability, security policy, and exact runtime qualification |
| gpt-oss | Widely adopted open-weight reasoning and agent workloads | 20B/120B GPU fit, reasoning protocol, and runtime version |
| Google Gemma | Efficient multimodal open-weight family | Model terms, multimodal runtime support, and artifact access |
...
provider contract with independent evidence. Check [compatibility and qualification](/compatibility) before
selecting a backend.
</Note>
