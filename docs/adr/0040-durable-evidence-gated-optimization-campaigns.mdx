...
- Queue activation as a separate durable, idempotent operation. New-endpoint activation may cross
  only the qualified boundary. Evolution activation delegates to the existing guarded rollout
  operation and may cross only `guard_passed`. Validate these states before any child mutation.
