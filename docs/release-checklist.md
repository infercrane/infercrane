...
after `v2.0.0-rc.1`, before any stable tag or public release. Never convert a deferred row to passed
...
- [ ] `make candidate-artifacts RELEASE_CANDIDATE_TAG=v2.0.0-rc.1` produces exactly four archives,
...
- [ ] `.release/evidence/v2.0.0-rc.1.md` records exact commands and artifacts.
- [ ] The worktree is clean and local annotated tag `v2.0.0-rc.1` points at the evidence commit.
...
- [ ] `scripts/qualify-v2-manual.sh status` reports `stage=complete` for the same commit; portable
      AWS/Kubernetes evidence remains attached to the consolidated provider report.
...
- [ ] Create stable tag `v2.0.0` only after every manual row is proven.
