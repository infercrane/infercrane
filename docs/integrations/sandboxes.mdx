...
This page describes **externally owned** sandboxes. For InferCrane-managed
lifecycle backed by a dedicated Brezel project, see
[Native Brezel sandboxes](/integrations/brezel-sandboxes).
...
<Warning>
These external-reference routes do not provide sandbox isolation and never
store sandbox commands, files, prompts, outputs, or provider credentials.
Configure those controls in the external sandbox system that owns execution.
</Warning>

## Console workflow

Open **Settings → Advanced integrations**. The console can issue, rotate, and revoke the
