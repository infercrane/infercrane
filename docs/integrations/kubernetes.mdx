...
KServe `LLMInferenceService` and llm-d remain separate integration boundaries. Dynamo uses the
explicit ownership contract in [ADR 0036](/adr/0036-dynamo-serving-topology-boundary) and a distinct
`kubernetes-dynamo` adapter; it is never enabled implicitly by `workload_api`.
