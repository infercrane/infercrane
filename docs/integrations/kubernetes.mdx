...
<Warning>
The v0.9 adapter has hermetic and Kind lifecycle qualification. Real GPU, vLLM, SGLang, and custom OCI
compatibility remains deferred to the consolidated v1 manual qualification. Registration is not a
...
KServe `LLMInferenceService`, llm-d, and Dynamo are not enabled in v0.9 because each can own routing,
