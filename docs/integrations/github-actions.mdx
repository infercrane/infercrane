...
<Warning>
  The action source is included in the v0.4 release candidate, but the public Git tag is not
...
jobs:
  plan:
    ...
    steps:
      ...
      - uses: infercrane/infercrane/actions/infercrane@v0.4.0-rc.1
        with:
          mode: plan
...
The check passes only when the expected revision is active, serving, converged, and has no active
operation. Output is stripped of terminal escapes, bounded for GitHub summaries, and redacts values
from credential-like environment variables. The action does not print or upload prompt content.
