...
The final command should be empty for a new boundary. GPU quota, current zone availability, and a
billing budget alert must be checked separately. Budget alerts notify; they do not stop Compute
Engine spend.
...
service account, worker secret, and immutable VM image. It fails when Private Google Access is not
enabled because workers intentionally have no public IP. It does not prove GPU stock, quota, the
private route from the control plane, or runtime readiness; those remain real-infrastructure checks.
