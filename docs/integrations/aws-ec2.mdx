...
The v0.3 AWS adapter provisions one EC2 instance per durable replica intent through Provider Contract
...
For the simulated v0.6 portable-runtime path, apply
...
Real GPU compatibility remains a deferred v1 manual gate.
...
Cost is reported as `unknown` in v0.3. InferCrane does not ship a live AWS pricing catalog and will not
...
until the consolidated v1 manual qualification; inspect `infercrane integrations` for the exact state.
