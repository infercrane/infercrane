...
| Evidence-gated optimization | In qualification | Offline proposal plus durable, idempotent campaigns, bounded expiring approval, fenced candidate transitions, exact capability compilation, and immutable optimized-artifact provenance pass local and PostgreSQL gates. LLM Compressor, ModelOpt, Speculators, TensorRT-LLM, LMCache, llm-d, and AIBrix retain external execution ownership. Automated campaign execution, real transformed artifacts, GPU qualification, and AISimulate migration remain deferred. |
