...
| Signed semantic evaluation evidence | Local-qualified | A strict evaluator-neutral result contract lets CI, Ragas, DeepEval, or custom suites produce aggregate evidence; `evaluation ingest` rejects unknown content fields, binds the result to an immutable revision, signs it, and optionally attaches it. Incompatible evidence fails closed and promotion remains a deterministic policy decision. External evaluator quality and key custody remain user responsibilities. |
| Artifact cache operations | Local-qualified | Inspect, observe, and durable prefetch workflows separate requested work from fresh provider observation. The AWS adapter validates exact-model encrypted EBS snapshots, attaches them at launch, and records bounded evidence; a real AWS cache-hit run remains external qualification. |
...
| Separate operations console | Private preview | Endpoint-first views, content-free monitoring with lifecycle overlays, durable operations, Request Inspector, Doctor, Release Guard, signed alerts, sourced FinOps and quality evidence, API-key/team management, encrypted local sessions, hosted identity boundaries, explicit partial states, responsive Playwright and accessibility tests pass locally; hosted deployment remains private. |
...
| Bare-metal GPU discovery | Local-qualified | `infercrane discover local` performs bounded, five-second, read-only NVIDIA inventory and hands off to observe-only endpoint adoption. It does not install an agent, manage processes, schedule GPUs, or claim runtime/model compatibility; AMD and remote discovery remain unavailable. |
...
| Model Recipes and Inference Lab | Local-qualified | Immutable artifact/revision/AIPerf recipes and tenant-scoped exact-workload comparisons support interactive, latency, throughput, and cost-efficiency objectives plus fail-closed performance, cost, region, and GPU-count constraints; public registry and real GPU catalog evidence are deferred. |
| Evidence-gated optimization | In qualification | Offline proposal plus durable, idempotent campaigns, bounded expiring approval, a restart-safe local execution coordinator, exact capability compilation, and immutable optimized-artifact provenance pass local and PostgreSQL gates. A measured-evidence barrier prevents proposal-order bias; exact-workload Lab ranking persists separately from benchmark, quality, and Release Guard evidence. Full-path constraints include TTFT, TPOT, error rate, goodput, throughput, region, GPU count, and sourced cost. The coordinator fails expired authority into cleanup, preserves lost-response adoption, rejects unknown rank/guard decisions, and stops before human promotion. LLM Compressor, ModelOpt, Speculators, TensorRT-LLM, LMCache, llm-d, and AIBrix retain external execution ownership. Production driver registration, real transformed artifacts, GPU qualification, and AISimulate migration remain deferred. |
| Inference Replay and Capacity Intelligence | In qualification | Content-free workload-shape capture, explicit AIPerf approximation, bounded cache observations, delegated prefetch intents, typed definite-versus-unknown provider results, tenant-scoped capacity evidence, and deterministic cache/readiness/reliability/cost placement are implemented. Real provider cache-hit timing and automatic multi-provider placement remain deferred. |
| FinOps and advisory Autopilot | Local-qualified | Currency-explicit, active-revision-bound OpenCost allocation evidence and benchmark price fallback produce immutable sourced hourly-cost reports; exact allocation selection prevents implicit cluster attribution. Savings remain unavailable without comparable evidence and approval never mutates production. Real billing reconciliation remains external qualification. |
...
| Admission, deadlines, and distributed request-rate quotas | Local-qualified | PostgreSQL reserves aggregate UTC-minute leases; gateways authorize from memory and fail closed without adding database reads to the inference path. Bounded concurrency/queueing, retry budgets, and one absolute endpoint deadline cover admission, every buffered attempt, and response streaming without replaying partial streams. Real-runtime cancellation remains exact-tuple qualification. |
...
| Managed external endpoint bindings | Local-qualified | Authenticated OpenRouter and generic OpenAI-compatible targets can participate in immutable endpoint plans with reference-only credentials, explicit consent, hard request/cost reservations, no replay, and candidate-only console staging. Real provider billing/protocol evidence remains deferred. |
| Reusable provider connections | Local-qualified | Tenant-scoped provider configuration records only target and secret references; endpoint bindings independently require privacy consent and hard budgets. Real provider protocol and billing semantics remain adapter-specific qualification. |
| LiteLLM gateway composition | Local-qualified | Existing LiteLLM OpenAI-compatible surfaces can be discovered and adopted without bundling or managing LiteLLM. InferCrane owns stable endpoint identity and evidence; real LiteLLM/plugin/provider combinations remain externally qualified. |
| External sandbox access | Local-qualified | Externally owned sandboxes can receive expiring credentials restricted to one endpoint. Rotation, revocation, control-API denial, alias filtering, and content-free persistence pass local black-box and adversarial tests; sandbox isolation and lifecycle remain external. |
| Signed training artifact handoff | Local-qualified | Ed25519-verified, content-free provenance binds one immutable external checkpoint to one tenant-scoped deployment revision. Training execution and registry availability remain external; promotion still requires compatible release evidence. |
| AWS EC2 BYOC | Experimental | Private-network, role-assumed, tag-owned, immutable-image lifecycle passes Provider Contract V1 hermetically. A complete Mistral 7B/A10G lifecycle and performance matrix is archived; other models, accelerators, disruption cases, and artifact-snapshot cache hits remain separate qualification boundaries. |
| GCP Compute BYOC | Experimental | Private-network, attached-identity, label-owned, immutable-image lifecycle passes hermetic adapter tests. Read-only preflight verifies every configured dependency and Private Google Access; guarded acceptance inventories VMs, boot disks, addresses, and forwarding rules. Real GCP GPU evidence is deferred. |
...
| NVIDIA Dynamo serving backend | Experimental | Immutable serving topology, one-parent DGD lifecycle, aggregated/disaggregated vLLM/SGLang manifests, KV-aware routing, and bounded aggregated-vLLM KVBM intent pass fixture and Kind API gates. Optimization proposals compile Dynamo as one fixed outer parent so InferCrane and Dynamo cannot become competing mutation owners. Dynamo operator, GPU, NIXL, cache, and performance evidence remains deferred. Planner, LMCache, and HiCache execution fail closed. |
| SkyPilot RunPod provisioning | Experimental | Requires credentialed elastic lifecycle acceptance and soak tests. |
| Production performance claims | In qualification | Exact Mistral 7B/vLLM/AWS A10G buffered, streaming, long-context, long-generation, and bounded-overload evidence is archived. No result is transferable to another model, runtime, GPU, provider, or cache state. |
