...
state_root=${INFERCRANE_QUALIFICATION_STATE_DIR:-"$root/.infercrane/provider-manual"}
run_id=${INFERCRANE_QUALIFICATION_RUN_ID:-}
approval=false

usage() {
  printf '%s\n' 'Usage: scripts/qualify-provider-manual.sh run|status|cleanup --approve-paid-resources'
  printf '%s\n' 'A fixed INFERCRANE_QUALIFICATION_RUN_ID makes the workflow resumable.'
}

command_name=${1:-}; shift || true
while [ "$#" -gt 0 ]; do case "$1" in --approve-paid-resources) approval=true;; *) usage >&2; exit 2;; esac; shift; done
[ -n "$run_id" ] || { printf '%s\n' 'INFERCRANE_QUALIFICATION_RUN_ID is required' >&2; exit 2; }
...
while :; do
  ...
  case "$stage" in
    ...
    complete) printf 'InferCrane provider qualification complete: %s\n' "$run_id"; exit 0;;
