...
grep -Fq 'serverless_max_workers=${INFERCRANE_ACCEPTANCE_SERVERLESS_MAX_WORKERS:-1}' "$root/scripts/release-acceptance.sh"
grep -Fq 'record serverless-plan ic plan "$MODEL" --name "$SERVERLESS_NAME" --compute serverless --cloud runpod --gpu "$GPU" --min 0 --max "$serverless_max_workers" --output json' "$root/scripts/release-acceptance.sh"
grep -Fq 'record preflight-doctor ic doctor --cloud --gpu "$GPU" --output json' "$root/scripts/release-acceptance.sh"
grep -Fq 'serverless|serverless-faults) default_paid_seconds=2400' "$root/scripts/release-acceptance.sh"
grep -Fq 'delete_if_present "$SERVERLESS_NAME" "$SERVERLESS_NAME-delete"' "$root/scripts/release-acceptance.sh"
