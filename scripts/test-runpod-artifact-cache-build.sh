...
printf '%s' "$plan" | jq -e '.mutation=="none" and .worst_case_cost_usd>0 and .prefetch_compute=="gpu" and .model_identity=="org/model@0123456789abcdef0123456789abcdef01234567"' >/dev/null
cpu_plan=$(env $common_env INFERCRANE_RUNPOD_PREFETCH_COMPUTE=cpu INFERCRANE_RUNPOD_CPU_HOURLY_USD=0.08 INFERCRANE_E2E_REPORT_DIR="$report" "$script" plan)
